...
local function resolvedState(durObj, mode, sourceID, start, duration, spellID, mirrorBacked, mirrorPayload, activity)
    ...
    elseif state.mode == "charge" then
        state.isOnCooldown = state.active == true
    else
        state.isOnCooldown = false
    end
    state.rechargeActive = activity.rechargeActive == true
        or (state.mode == "charge" and state.active == true and durObj ~= nil)
    state.hasCharges = activity.hasCharges == true or state.mode == "charge"
...
assert(applied == true, "renderer should preserve a prior real mirror binding over factual gcd-only state")
assert(priorMirrorAppliedDuration == nil,
    "renderer preservation should dedupe an already-bound prior mirror DurationObject")
assert(priorMirrorResourceBlockedIcon._lastDurObj == preservedMirrorDuration,
    "renderer preservation should keep the prior mirror DurationObject")
assert(priorMirrorResourceBlockedIcon._resolvedCooldownMode == "cooldown",
    "renderer preservation should keep real-cooldown mode on the icon")
assert(priorMirrorResourceBlockedIcon._lastDurObjKey == "cooldown:mirror:27927:2218",
    "renderer preservation should keep the prior mirror source key")
assert(priorMirrorResourceBlockedIcon._hasCooldownActive == true,
    "renderer-preserved prior mirror should remain cooldown-active")
assert(priorMirrorDesaturated == true,
    "renderer-preserved prior mirror should remain desaturated")
