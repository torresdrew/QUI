...
# taint-check.yml strict + self-test jobs):
...
#   6. tooling unit tests            tests/api-docs/extract_test.lua,
...
echo "== gate 1/6: compile check (luac 5.1) =="
bash tools/check_compile.sh || fail=1

echo "== gate 2/6: taint analyzer (strict) =="
"$LUA_BIN" tools/test_taint.lua --strict-only || fail=1

echo "== gate 3/6: taint analyzer unit tests (tests/taint/) =="
taint_fail=0; taint_count=0
...
[ "$taint_fail" -eq 0 ] && echo "taint tests: $taint_count files passed" || fail=1

echo "== gate 4/6: profile fixture tests =="
"$LUA_BIN" tools/test_profiles.lua || fail=1

echo "== gate 5/6: unit tests (tests/unit/) =="
...
echo "== gate 6/6: tooling unit tests =="
tool_fail=0
for t in tests/api-docs/extract_test.lua tools/lua_defs_gen_test.lua; do
  if ! "$LUA_BIN" "$t" >/dev/null 2>&1; then
    echo "FAIL: $t"
    "$LUA_BIN" "$t" || true   # re-run loudly so the error is visible
    tool_fail=1
...
[ "$tool_fail" -eq 0 ] && echo "tooling tests: 2 files passed" || fail=1
