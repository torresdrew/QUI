          ...
          const recordData = fieldsInfo.map((fieldInfo, fieldIndex) => {
            switch (fieldInfo.storageType) {
              ...
              case "bitpackedIndexedArray": {
                ...
                assert__default(typeof value === "number", "Bitpacked value must be a number");
...
class CASCClient {
  ...
  async init() {
    ...
    assert__default(serverConfig, "No server config found");
  ...
  async loadRemoteTACTKeys() {
    ...
    lines.forEach((line) => {
      ...
      assert__default(keyHex.length === 32, `Invalid key length: ${keyHex.length.toString()}`);
  ...
  async loadTACTKeys() {
    ...
    assert__default(keysCKeys?.[0], "Failing to find dbfilesclient/tactkey.db2");
    assert__default(lookupCKeys?.[0], "Failing to find dbfilesclient/tactkeylookup.db2");
    ...
    lookupReader.getAllIDs().forEach((keyID) => {
      ...
      if (keyRow) {
        ...
        assert__default(keyName.length === 16, `Invalid keyName length: ${keyName.length.toString()}`);
        assert__default(keyHexLE.length === 32, `Invalid key length: ${keyHexLE.length.toString()}`);
        ...
        if (column.isID) {
          ...
          if (cell.type === "bitpackedArray") {
            ...
            if (cell.data > 0) {
              ...
              assert__default(typeof cell.string === "string", `Missing string for string column ${column.name}`);
