...
writes DrewUI_Locale_<loc>/<loc>.lua, and updates state.json.
...
def read_enus_keys(path=ENUS):
    ...
    proc = subprocess.run(
        [lua_bin(), "-e",
         f'local ns = {{}}; assert(loadfile("{quoted}"))("DrewUI", ns);'
...
def read_overlay(loc, enus_keys=None):
    ...
    proc = subprocess.run(
        [lua_bin(), "-e",
         ...
         f'local ns = {{}}; assert(loadfile("{path}"))("DrewUI", ns);'
...
def overlay_path(loc):
    ...
    DrewUI_OptionsSearch_<loc> folders; writing anywhere else produced a file
...
def overlay_source(loc, table, generator):
    ...
    All ten overlays are listed in DrewUI.toc, so nine of them load and return
    ...
    """
    lines = ['local want = (DrewUIDB and DrewUIDB.global and DrewUIDB.global.selectedLocale) or GetLocale()',
...
_TRANSLATE_SYSTEM = (
    ...
    "proper nouns that ship untranslated in the {lang} WoW client (keep 'DrewUI').\n"
...
def real_translate(loc, items):
    ...
    model = os.environ.get("DrewUI_I18N_MODEL", "claude-opus-5")
