...
local function GetTextureList()
    local LSM = ns.LSM
    local list = { { value = nil, text = ns.L["(default)"] } }
    if LSM then
...
BuildNativeDamageMeterTab = function(tabContent)
    ...
    if GUI and GUI.SetSearchContext then
        GUI:SetSearchContext({
            ...
            subTabName   = ns.L["Damage Meter (Native)"],
    ...
    for id, w in pairs(native.windows or {}) do
        local displayName = (w and w.name and w.name ~= "") and w.name or (ns.L["Window "] .. id)
        table.insert(editingOptions, { value = id, text = displayName .. ns.L[" (per-window overrides)"] })
    end
    ...
    local editW = GUI:CreateFormDropdown(sEdit.frame, nil, editingOptions, "editing", state, function()
        ...
        RebuildPage()
    end, { description = ns.L["Pick 'Global' to edit shared appearance, or a specific window to set per-window overrides. Each appearance widget below shows an Override toggle in per-window mode."] })
    ...
    local visibilityOptions = {
        { text = ns.L["Always"],    value = "always"   },
        { text = ns.L["In Combat"], value = "inCombat" },
        { text = ns.L["Hidden"],    value = "hidden"   },
    }
    local visW = GUI:CreateFormDropdown(sBeh.frame, nil, visibilityOptions, "visibility", native, ApplyNative,
        { description = ns.L["When the native meter is visible. Always = shown at all times; In Combat = only during combat; Hidden = meter exists but is invisible."] })
    local refCW = GUI:CreateFormSlider(sBeh.frame, nil, 0.1, 2.0, 0.1, "refreshRateCombat", native, ApplyNative,
        { description = ns.L["How often (seconds) the meter refreshes in combat (0.1-2.0)."] })
    sBeh.AddRow(row(sBeh.frame, ns.L["Visibility"], visW), row(sBeh.frame, ns.L["Refresh Rate (Combat)"], refCW))
    ...
    local numberFormatOptions = {
        ...
        { value = "complete", text = ns.L["Complete (1,500 / 2,400,000)"] },
    }
    local iconStyleOptions = {
        { value = "spec",  text = ns.L["Spec icon (when available)"] },
        { value = "class", text = ns.L["Class icon"] },
    ...
    local numW = BuildOverrideWidget(sBeh.frame, app, "numberFormat", ApplyNative,
        function(p, bt)
            return GUI:CreateFormDropdown(p, nil, numberFormatOptions, "numberFormat", bt, ApplyNative,
                { description = ns.L["How damage / healing values are formatted in row text and tooltips."] })
        end)
    local iconW = BuildOverrideWidget(sBeh.frame, app, "iconStyle", ApplyNative,
        function(p, bt)
            return GUI:CreateFormDropdown(p, nil, iconStyleOptions, "iconStyle", bt, ApplyNative,
                { description = ns.L["Which icon to show on each row."] })
        end)
    local showSecW = BuildOverrideWidget(sBeh.frame, app, "showSecondaryValue", ApplyNative,
        function(p, bt)
            return GUI:CreateFormCheckbox(p, nil, "showSecondaryValue", bt, ApplyNative,
                { description = ns.L["When ON (default), rows show the other metric in parentheses — e.g. DPS mode shows 1.5M (42.3M). Turn off to show only the primary value."] })
        end)
    local pending = placeOverrideRow(sBeh, ns.L["Number Format"], numW, nil)
    pending = placeOverrideRow(sBeh, ns.L["Icon Style"], iconW, pending)
    pending = placeOverrideRow(sBeh, ns.L["Show Secondary Value"], showSecW, pending)
    ...
    local hW = override(sBars.frame, "barHeight", function(p, bt)
        return GUI:CreateFormSlider(p, nil, 12, 30, 1, "barHeight", bt, ApplyNative,
            { description = ns.L["Pixel height of each damage row (12-30)."] })
    end)
    local spaceW = override(sBars.frame, "barSpacing", function(p, bt)
        return GUI:CreateFormSlider(p, nil, 0, 8, 1, "barSpacing", bt, ApplyNative,
            { description = ns.L["Pixel gap between rows (0-8)."] })
    end)
    pending = placeOverrideRow(sBars, ns.L["Bar Height"], hW, nil)
    pending = placeOverrideRow(sBars, ns.L["Bar Spacing"], spaceW, pending)
    ...
    local texW = BuildNestedOverrideWidget(sBars.frame, app, "textures", "bar", ApplyNative,
        function(p, bt)
            return GUI:CreateFormDropdown(p, nil, textures, "bar", bt, ApplyNative,
                { description = ns.L["LSM statusbar texture for the bar fill. (default) keeps the QUI built-in WHITE8x8."] })
        end)
    ...
    local rowBgW = override(sBars.frame, "showRowBackground", function(p, bt)
        return GUI:CreateFormCheckbox(p, nil, "showRowBackground", bt, ApplyNative,
            { description = ns.L["Show the dark trough behind each row's colored fill."] })
    end)
    ...
    local colorW = override(sBars.frame, "barColor", function(p, bt)
        return GUI:CreateFormColorPicker(p, nil, "barColor", bt, ApplyNative, nil,
            { description = ns.L["Custom bar color used when both Use Class Color and Use Accent are off."] })
    end)
    pending = placeOverrideRow(sBars, ns.L["Custom Bar Color"], colorW, pending)
    ...
    local function BuildFontSlotCard(slotKey, label)
        ...
        if editingWindowID > 0 then
            ...
            s.AddRow(row(s.frame, string.format(ns.L["Override %s font slot"], label), slotToggle))
        end

        local slot, enabled = ResolveFontSlot(slotKey)

        local fontW = GUI:CreateFormDropdown(s.frame, nil, fontList, "name", slot, ApplyNative,
            { description = string.format(ns.L["LSM font for %s."], label) })
    ...
    local bgW = nestedColor(sCol.frame, "bg", ns.L["Background fill color for the meter window."])
    local headerColorW = nestedColor(sCol.frame, "headerText", ns.L["Color of the meter-type label and session timer. (Default: QUI accent.)"])
    pending = placeOverrideRow(sCol, ns.L["Window Background"], bgW, nil)
    pending = placeOverrideRow(sCol, ns.L["Header Text"], headerColorW, pending)

    local rowNameW = nestedColor(sCol.frame, "rowName", ns.L["Color of the player-name text on each row."])
    local rowValueW = nestedColor(sCol.frame, "rowValue", ns.L["Color of the damage / healing number on each row."])
    pending = placeOverrideRow(sCol, ns.L["Row Name"], rowNameW, pending)
    pending = placeOverrideRow(sCol, ns.L["Row Value"], rowValueW, pending)

    local borderW = nestedColor(sCol.frame, "border", ns.L["Window border color. (Default: QUI accent.)"])
    pending = placeOverrideRow(sCol, ns.L["Border"], borderW, pending)
    ...
    for _, id in ipairs(ids) do
        local ws = windows[id]
        local typeLabel = TYPE_NAMES[ws.damageMeterType] or (ns.L["Type "] .. tostring(ws.damageMeterType))
        local sessionLabel = SESSION_NAMES[ws.sessionType] or (ns.L["Session "] .. tostring(ws.sessionType))
        local autoName = string.format(ns.L["Window %1$d — %2$s / %3$s"], id, typeLabel, sessionLabel)
        local windowID = id
        ...
        local nameEdit = GUI:CreateFormEditBox(windowsFrame, nil, "name", ws, function()
            ...
            { description = ns.L["Display name for this window. Leave blank to auto-name from the window's metric and session type."] })
        ...
        GUI:AttachTooltip(delBtn, ns.L["Permanently remove this window. Its layout and per-window overrides are lost."], ns.L["Delete Window"])

        wy = wy - FORM_ROW
    end

    local count = #ids
    local addLabel = (count >= 5) and ns.L["+ Add Window  (cap is 5)"] or ns.L["+ Add Window"]
    ...
    GUI:AttachTooltip(addBtn,
        ns.L["Create a new damage meter window. The new window starts with global appearance and can be customized via the Editing Window selector. Up to 5 windows are supported."],
        ns.L["Add Window"])
