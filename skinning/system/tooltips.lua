...
local pairs, ipairs, type, pcall, select = pairs, ipairs, type, pcall, select
...
local function ApplyOverlayBackdrop(skinFrame, edgeSize, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    -- Clear first for deterministic backdrop updates.
    skinFrame:SetBackdrop(nil)
    skinFrame:SetBackdrop(backdrop)
...
local function StripCompareHeader(tooltip)
    ...
    if header.NineSlice then
        HideNineSlice(header.NineSlice)
    end
