...
local function BuildBuffsSettings(content, gfdb, onChange)
    ...
    CreateComposerCollapsible(content, "Buff Filtering", function(body, updateH)
        ...
        local c2 = GUI:CreateFormCheckbox(classificationContainer, "Raid (In Combat)", "raidInCombat", buffClass, syncedOnChange, { description = "Include buffs flagged as raid-relevant only while in combat. A narrower version of Raid for users who don't want non-combat noise." }); c2:SetPoint("TOPLEFT", 0, classY); c2:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
        local c3 = GUI:CreateFormCheckbox(classificationContainer, "Cancelable", "cancelable", buffClass, syncedOnChange, { description = "Include buffs you can right-click to cancel (mostly your own player buffs)." }); c3:SetPoint("TOPLEFT", 0, classY); c3:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
        local c4 = GUI:CreateFormCheckbox(classificationContainer, "Not Cancelable", "notCancelable", buffClass, syncedOnChange, { description = "Include buffs you can't right-click to cancel (most buffs from other players, NPCs, or the environment)." }); c4:SetPoint("TOPLEFT", 0, classY); c4:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
        ...
        local c6 = GUI:CreateFormCheckbox(classificationContainer, "Big Defensive", "bigDefensive", buffClass, syncedOnChange, { description = "Include big personal defensives (e.g. Ice Block, Divine Shield)." }); c6:SetPoint("TOPLEFT", 0, classY); c6:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
        local c7 = GUI:CreateFormCheckbox(classificationContainer, "External Defensive", "externalDefensive", buffClass, syncedOnChange, { description = "Include externally-cast defensives applied to a unit (e.g. Pain Suppression, Ironbark)." }); c7:SetPoint("TOPLEFT", 0, classY); c7:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
...
local function BuildDebuffsSettings(content, gfdb, onChange)
    ...
    CreateComposerCollapsible(content, "Debuff Filtering", function(body, updateH)
        ...
        local d2 = GUI:CreateFormCheckbox(classificationContainer, "Raid (In Combat)", "raidInCombat", debuffClass, syncedOnChange, { description = "Include debuffs flagged as raid-relevant only while in combat." }); d2:SetPoint("TOPLEFT", 0, classY); d2:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
        local d3 = GUI:CreateFormCheckbox(classificationContainer, "Crowd Control", "crowdControl", debuffClass, syncedOnChange, { description = "Include crowd-control debuffs (stuns, fears, roots, silences, etc.)." }); d3:SetPoint("TOPLEFT", 0, classY); d3:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
        local d4 = GUI:CreateFormCheckbox(classificationContainer, "Important", "important", debuffClass, syncedOnChange, { description = "Include debuffs flagged by Blizzard as important (key mechanics and notable effects)." }); d4:SetPoint("TOPLEFT", 0, classY); d4:SetPoint("RIGHT", classificationContainer, "RIGHT", 0, 0); classY = classY - FORM_ROW
        classificationContainer:SetHeight(math.abs(classY))
        sectionRelayouts[#sectionRelayouts + 1] = L:Finish()
    end, sections, relayout)
...
local function CreateHealthTintAnimationPreview(parent, indicator)
    ...
    local label = GUI:CreateLabel(preview, "Animation Preview", 11, C and C.textMuted)
            ...
            if selectedEntry then
                ...
                if selectedIndicator then
                    ...
                    elseif selectedIndicator.type == "healthBarColor" then
                        ...
                        AddDetailWidget(GUI:CreateFormDropdown(detailArea, "Tint Animation", HEALTH_TINT_ANIMATION_OPTIONS, "animation", selectedIndicator, onChange), DROP_ROW)
                        AddDetailWidget(CreateHealthTintAnimationPreview(detailArea, selectedIndicator), 78)
                    else
...
local function CreateDesignerPreview(container, previewType, childRefs)
    ...
    if aiDB.enabled then
        ...
        if firstTint then
            local tintOverlay = CreateFrame("StatusBar", nil, healthBar)
            tintOverlay:SetAllPoints(healthBar)
            tintOverlay:SetFrameLevel(healthBar:GetFrameLevel() + 1)
            tintOverlay:SetStatusBarTexture(texturePath)
            tintOverlay:SetOrientation(healthBar.GetOrientation and healthBar:GetOrientation() or "HORIZONTAL")
            tintOverlay:SetMinMaxValues(0, 1)
            tintOverlay:SetValue(0.72)
            tintOverlay:SetStatusBarColor(firstTint[1] or 0.2, firstTint[2] or 0.8, firstTint[3] or 0.2, firstTint[4] or 1)
        end
    end
