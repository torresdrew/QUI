...
function QUI_LayoutMode:Open()
    ...
    -- Ensure deferred Layout Mode subsystems are loaded (UI, settings,
    -- providers, composer). No-op if already loaded or nothing registered.
    ns.EnsureDeferredGroup("layoutmode")
...
AddHandleScripts = function(handle, def)
    ...
    handle:SetScript("OnDragStart", function(self)
        ...
        if fa and fa[self._barKey] and type(fa[self._barKey]) == "table" then
            ...
            hasActiveAnchor = parent and parent ~= "disabled" and parent ~= "screen"
            if hasActiveAnchor and not QUI_LayoutMode._elements[parent] then
                -- Parent no longer exists in layout mode; treat as detached/orphaned.
        ...
        if not anchorKey and self._wasAnchoredOnDragStart and not self._shiftDragStart then
            ...
            if pending and pending.anchorTarget then
                ...
                if fa and fa[self._barKey] and type(fa[self._barKey]) == "table" then
                    ...
                    if entry.parent and entry.parent ~= "disabled" and entry.parent ~= "screen"
                        and QUI_LayoutMode._elements[entry.parent] then
            ...
            um:RegisterElement({
                ...
                setEnabled = function(val)
                    ...
                    if info.refresh then
                        ns.Registry:Refresh(info.refresh)
                    end
                end,
...
do
    local function RegisterReadyCheckProvider()
        ...
        local function RefreshColors()
            ns.Registry:Refresh("skinReadyCheck")
        end
...
do
    local function RegisterMPlusTimerProvider()
        ...
        local function RefreshColors()
            ns.Registry:Refresh("skinMPlusTimer")
        end
...
do
    local function RegisterObjectiveTrackerProvider()
        ...
        local function RefreshOT()
            ns.Registry:Refresh("skinObjectiveTracker")
        end
...
-- Hook module refreshes via Registry to sync layout positions when Layout Mode is active.
local function LayoutSyncHook()
    if QUI_LayoutMode.isActive then
        DebouncedLayoutSync()
    end
end
ns.Registry:HookRefresh("unitframes", LayoutSyncHook)
ns.Registry:HookRefresh("castbar", LayoutSyncHook)
ns.Registry:HookRefresh("ncdm", LayoutSyncHook)
ns.Registry:HookRefresh("buffbar", LayoutSyncHook)
ns.Registry:HookRefresh("customTrackers", LayoutSyncHook)
ns.Registry:HookRefresh("brezCounter", LayoutSyncHook)
ns.Registry:HookRefresh("combatTimer", LayoutSyncHook)
ns.Registry:HookRefresh("xpTracker", LayoutSyncHook)
