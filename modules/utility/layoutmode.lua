...
-- Anchor indicator (replaces Unicode ⚓ which WoW fonts can't render)
local ANCHOR_INDICATOR = "|cff34d399>>|r"
        ...
        if not self._handles[key] then
            ...
            C_Timer.After(0, function()
                ...
                if targetFrame and targetFrame:IsShown() then
                    ...
                    targetFrame:SetFrameStrata(handle:GetFrameStrata())
                    targetFrame:SetFrameLevel(math.max(1, handle:GetFrameLevel() - 50))
...
function QUI_LayoutMode:Open()
    ...
    if InCombatLockdown() then
        print("|cff34D399QUI:|r Cannot open Layout Mode during combat.")
        return
    end
    ...
    C_Timer.After(0, function()
        for hKey, handle in pairs(self._handles) do
            if handle:IsShown() and not handle._isChildOverlay then
    ...
    if not self._firstOpenDone then
        ...
        print("|cff34D399QUI Layout Mode:|r Drag to move | Click to select | Arrow keys to nudge | Shift+Drag near edge = anchor | Escape to close")
    end
end
...
function QUI_LayoutMode:GetSelectedKey()
...
local function GetFrameAnchoring()
...
CommitPositions = function()
    if InCombatLockdown() then
        print("|cff34D399QUI:|r Cannot save positions during combat. Try again after combat ends.")
        return
    end
        ...
        if def then
            ...
            elseif fa then
                ...
                if pos.anchorTarget then
                    -- Anchor to another element (Shift+snap)
                    fa[key].parent = pos.anchorTarget
                    fa[key].point = pos.anchorPointSelf or "CENTER"
                    fa[key].relative = pos.anchorPointTarget or "CENTER"
                else
                    -- Absolute positioning (screen)
                    fa[key].parent = "screen"
                    fa[key].point = pos.point
                    fa[key].relative = pos.relPoint
                end
...
RevertPositions = function()
    if InCombatLockdown() then
        print("|cff34D399QUI:|r Cannot revert positions during combat.")
        return
    end
...
AddHandleVisuals = function(handle, def)
    ...
    -- Anchor label (shown when anchored via Shift+snap or existing anchor)
    local anchorLabel = handle:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    anchorLabel:SetPoint("BOTTOM", handle, "TOP", 0, 2)
    anchorLabel:SetTextColor(ACCENT_R, ACCENT_G, ACCENT_B, 1)
    anchorLabel:Hide()
    handle._anchorLabel = anchorLabel
end
...
AddHandleScripts = function(handle, def)
    ...
    handle:SetScript("OnDragStart", function(self)
        ...
        -- Anchored frames require Shift to drag (prevents accidental detach)
        if self._isAnchored and not IsShiftKeyDown() then
            -- Flash border red briefly to indicate locked state
        ...
        if self._isChildOverlay then
            -- Child overlay: drag the parent frame
            self._parentFrame:StartMoving()
        else
            -- Proxy mover: drag the mover itself
            self:StartMoving()
        end

        -- Start OnUpdate for live repositioning + snap + coords
        self:SetScript("OnUpdate", function(frame)
            if frame._isChildOverlay then
                -- Parent is already moving — just update coords and snap
            else
                -- Proxy: live-reposition the actual frame to follow mover
                local key = frame._barKey
                local def2 = QUI_LayoutMode._elements[key]
                if def2 then
                    local targetFrame = def2.getFrame and def2.getFrame()
                    if targetFrame then
                        local ox, oy = HandleToOffsets(frame)
                        -- Subtract center offset so frame tracks its own center
            ...
            local ox, oy = HandleToOffsets(frame)
            frame._coords:SetText(string.format("X: %d  Y: %d", ox, oy))

            -- Snap system
            local ui = ns.QUI_LayoutMode_UI
            if ui and ui.snapEnabled and ui.ApplySnap then
                ui:ApplySnap(frame)
            end
        end)
    ...
    handle:SetScript("OnDragStop", function(self)
        if self._isChildOverlay then
            self._parentFrame:StopMovingOrSizing()
        else
        ...
        if anchorKey then
            ...
            local info = ns.FRAME_ANCHOR_INFO and ns.FRAME_ANCHOR_INFO[anchorKey]
            local displayName = info and info.displayName or anchorKey
            self._coords:SetText(string.format(ANCHOR_INDICATOR .. " %s  X: %d  Y: %d", displayName, ox, oy))
            -- Keep anchor label visible after drag
            if self._anchorLabel then
                self._anchorLabel:SetText(ANCHOR_INDICATOR .. " " .. displayName)
                self._anchorLabel:Show()
            end
            ...
            -- Clear anchor label if no anchor
            if self._anchorLabel then
                self._anchorLabel:Hide()
            end
        ...
        -- Re-show settings panel after drag (same key, won't reposition)
...
SyncHandle = function(key)
    ...
    if handle._isChildOverlay then
        ...
        w = math.max(w or HANDLE_MIN_SIZE, HANDLE_MIN_SIZE)
        h = math.max(h or HANDLE_MIN_SIZE, HANDLE_MIN_SIZE)
        handle:SetSize(w, h)
        ...
        -- Position: check pending first, then saved, then frame position
        local pending = QUI_LayoutMode._pendingPositions[key]
        if pending then
            handle:ClearAllPoints()
    ...
    -- Update coordinate text and show existing anchor labels
    ...
    if existingAnchorKey then
        ...
        local info = ns.FRAME_ANCHOR_INFO and ns.FRAME_ANCHOR_INFO[existingAnchorKey]
        local displayName = info and info.displayName or existingAnchorKey
        handle._coords:SetText(string.format(ANCHOR_INDICATOR .. " %s  X: %d  Y: %d", displayName, ox, oy))
        if handle._anchorLabel then
            handle._anchorLabel:SetText(ANCHOR_INDICATOR .. " " .. displayName)
            handle._anchorLabel:Show()
        end
...
local function OnProfileChanged()
    if QUI_LayoutMode.isActive then
        ...
        print("|cff34D399QUI:|r Profile changed. Layout Mode closed — reopen to use new profile positions.")
    end
end
...
do
    local function RegisterDisplayElements()
        ...
        local DISPLAY_ELEMENTS = {
            ...
            { key = "buffFrame",       label = "Buff Frame",        frame = "BuffFrame",              order = 3 },
            { key = "debuffFrame",     label = "Debuff Frame",      frame = "DebuffFrame",            order = 4 },
    ...
    local function RegisterQoLElements()
        ...
        local QOL_ELEMENTS = {
            ...
            },
            {
                ...
                frame = nil, -- uses Blizzard TotemFrame
                blizzFrame = "TotemFrame",
                dbKey = "totemBar", enabledField = "enabled",
                refresh = "QUI_RefreshTotemBar",
                previewOn  = function() if _G.QUI_ToggleTotemBarPreview then _G.QUI_ToggleTotemBarPreview() end end,
    ...
    local function RegisterMPlusTimerProvider()
        ...
        local function BuildMPlusTimerSettings(content, key, width)
            ...
            if mpDB.borderColor == nil then
                local fb = general and (general.skinBorderColor or general.addonAccentColor) or { 0.204, 0.827, 0.6, 1 }
    ...
    local function RegisterObjectiveTrackerProvider()
        ...
        local function BuildObjectiveTrackerSettings(content, key, width)
            ...
            U.CreateCollapsible(content, "Skinning", 1 * FORM_ROW + 8, function(body)
                ...
                P(GUI:CreateFormCheckbox(body, "Skin Objective Tracker", "skinObjectiveTracker", general, function()
                    GUI:ShowConfirmation({
                        title = "Reload UI?",
                        message = "Skinning changes require a reload to take effect.",
                        acceptText = "Reload",
