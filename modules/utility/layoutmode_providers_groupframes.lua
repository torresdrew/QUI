...
local function BuildFrameSettings(content, contextMode, sections, relayout)
    ...
    local function onChange()
        -- RefreshGF calls Registry:Refresh("groupframes") which already triggers
    ...
    U.CreateCollapsible(content, "Layout", 1, function(body)
        ...
        if isRaid then
            local function onGroupByChange()
                onChange()
                -- Group By toggles additional controls; force a panel rebuild so
                -- Layout section content/height immediately matches the mode.
                if settingsPanel and settingsPanel.IsShown and settingsPanel:IsShown()
                    and settingsPanel._currentKey == syncKey and settingsPanel.Show then
                    settingsPanel._currentKey = nil
                    settingsPanel:Show(syncKey)
                end
            end
            sy = P(GUI:CreateFormDropdown(body, "Group By", GROUP_BY_OPTIONS, "groupBy", layout, onGroupByChange), body, sy)
            local groupBy = layout.groupBy or "GROUP"
            if groupBy == "NONE" then
...
-- (Layout Mode Open), replacing the original C_Timer.After(3.1, ...) delay.
