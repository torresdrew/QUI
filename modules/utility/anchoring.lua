...
local FRAME_RESOLVERS = {
    ...
    rotationAssistIcon = function()
        ...
        if ns.Registry:Has("rotationAssist") then
            ns.Registry:Refresh("rotationAssist")
...
local FRAME_ANCHOR_INFO = {
    ...
    datatextPanel   = { displayName = "Datatext Panel",        category = "Display",           order = 6 },
    ...
    elseif CASTBAR_ANCHOR_KEYS[key] then
        ...
        if chainSettings then
            settings = {
                ...
                sizeStable = settings.sizeStable,
            }
        end
    end
...
function QUI_Anchoring:ApplyAllFrameAnchors(force)
    ...
    -- Clear frame-anchoring ownership before re-applying from current profile.
    -- Keep anchoredFrames intact; it also tracks runtime registrations created
    -- by RegisterAnchoredFrame(), which are independent of frameAnchoring DB.
...
-- Hook module refreshes via Registry to reapply position overrides after
-- modules reposition frames. Registry:HookRefresh fires after any Refresh call.
local anchoringHook = DebouncedReapplyOverrides
ns.Registry:HookRefresh("castbar", anchoringHook)
ns.Registry:HookRefresh("unitframes", anchoringHook)
ns.Registry:HookRefresh("groupframes", anchoringHook)
ns.Registry:HookRefresh("ncdm", anchoringHook)
ns.Registry:HookRefresh("buffbar", anchoringHook)
ns.Registry:HookRefresh("raidbuffs", anchoringHook)
ns.Registry:HookRefresh("customTrackers", anchoringHook)
ns.Registry:HookRefresh("brezCounter", anchoringHook)
ns.Registry:HookRefresh("combatTimer", anchoringHook)
ns.Registry:HookRefresh("rangecheck", anchoringHook)
ns.Registry:HookRefresh("xpTracker", anchoringHook)
ns.Registry:HookRefresh("actionTracker", anchoringHook)
ns.Registry:HookRefresh("skyriding", anchoringHook)
ns.Registry:HookRefresh("petWarning", anchoringHook)
ns.Registry:HookRefresh("focusCastAlert", anchoringHook)
