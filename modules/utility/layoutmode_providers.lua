...
local function RegisterAllProviders()
    ...
    settingsPanel:RegisterProvider("xpTracker", { build = function(content, key, width)
        ...
        local function Refresh() ns.Registry:Refresh("xpTracker") end
    ...
    settingsPanel:RegisterProvider("combatTimer", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("combatTimer") end
    ...
    settingsPanel:RegisterProvider("brezCounter", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("brezCounter") end
    ...
    settingsPanel:RegisterProvider("rotationAssistIcon", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("rotationAssist") end
    ...
    settingsPanel:RegisterProvider("focusCastAlert", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("focusCastAlert") end
    ...
    settingsPanel:RegisterProvider("buffFrame", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("buffBorders") end

        U.CreateCollapsible(content, "Borders", 3 * FORM_ROW + 22, function(body)
            local sy = -4
            ...
            note:SetText("Border Size and Font Size are shared with Debuffs")
            note:SetJustifyH("LEFT")
        end, sections, relayout)

        U.CreateCollapsible(content, "Layout", 6 * FORM_ROW + 22, function(body)
            local sy = -4
            ...
            P(GUI:CreateFormCheckbox(body, "Grow Up", "buffGrowUp", bb, Refresh), body, sy)
            local note = body:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            note:SetPoint("BOTTOMLEFT", body, "BOTTOMLEFT", 4, 4)
            note:SetPoint("RIGHT", body, "RIGHT", -4, 0)
            note:SetTextColor(0.6, 0.6, 0.6, 0.8)
            note:SetText("Set sliders to 0 to use Blizzard defaults")
            note:SetJustifyH("LEFT")
        end, sections, relayout)
    ...
    settingsPanel:RegisterProvider("debuffFrame", { build = function(content, key, width)
        ...
        U.CreateCollapsible(content, "Borders", 3 * FORM_ROW + 22, function(body)
            ...
            note:SetPoint("RIGHT", body, "RIGHT", -4, 0)
            note:SetTextColor(0.6, 0.6, 0.6, 0.8)
            note:SetText("Border Size and Font Size are shared with Buffs")
            note:SetJustifyH("LEFT")
        end, sections, relayout)
        ...
        U.CreateCollapsible(content, "Layout", 6 * FORM_ROW + 22, function(body)
            ...
            P(GUI:CreateFormCheckbox(body, "Grow Up", "debuffGrowUp", bb, Refresh), body, sy)
            local note = body:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            note:SetPoint("BOTTOMLEFT", body, "BOTTOMLEFT", 4, 4)
            note:SetPoint("RIGHT", body, "RIGHT", -4, 0)
            note:SetTextColor(0.6, 0.6, 0.6, 0.8)
    ...
    settingsPanel:RegisterProvider("minimap", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("minimap") end
        local function RefreshUIHider() ns.Registry:Refresh("uiHider") end
        ...
        U.CreateCollapsible(content, "Button Drawer", 17 * FORM_ROW + 8, function(body)
            ...
            sy = P(GUI:CreateFormSlider(body, "X Offset", -200, 200, 1, "offsetX", drawer, Refresh), body, sy)
            sy = P(GUI:CreateFormSlider(body, "Y Offset", -200, 200, 1, "offsetY", drawer, Refresh), body, sy)
            sy = P(GUI:CreateFormSlider(body, "Toggle X Offset", -200, 200, 1, "toggleOffsetX", drawer, Refresh), body, sy)
            sy = P(GUI:CreateFormSlider(body, "Toggle Y Offset", -200, 200, 1, "toggleOffsetY", drawer, Refresh), body, sy)
            ...
            P(GUI:CreateFormSlider(body, "Button Spacing", 0, 20, 1, "buttonSpacing", drawer, Refresh), body, sy)
        end, sections, relayout)
    ...
    settingsPanel:RegisterProvider("extraActionButton", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("actionbars", "extraButtons") end
    ...
    settingsPanel:RegisterProvider("totemBar", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("totemBar") end
    ...
    ---------------------------------------------------------------------------
    for _, providerKey in ipairs({"rangeCheck", "actionTracker", "crosshair",
    ...
    for _, cbInfo in ipairs(CASTBAR_UNITS) do
        settingsPanel:RegisterProvider(cbInfo.key, { build = function(content, key, width)
            ...
            local function relayout() U.StandardRelayout(content, sections) end
            local function Refresh()
                ns.Registry:Refresh("castbar", "single", unitKey)
            end
    ...
    settingsPanel:RegisterProvider("skyriding", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("skyriding") end
    ...
    settingsPanel:RegisterProvider("partyKeystones", { build = function(content, key, width)
        ...
        local sections = {}
        local function relayout() U.StandardRelayout(content, sections) end
        local function Refresh() ns.Registry:Refresh("keyTracker") end
...
-- Providers are registered synchronously when the deferred group loads
-- (Layout Mode Open), replacing the original C_Timer.After(3, ...) delay.
