...
local SNAP_THRESHOLD_ANCHOR = 12  -- wider threshold when Shift held for anchoring
...
function QUI_LayoutMode_UI:Show()
    ...
    if self._nudgeFrame then
        ...
        -- Keyboard enabled only when a mover is selected (see OnSelectionChanged)
...
-- Snap guide color (blue tint, distinct from accent green)
local SNAP_GUIDE_R, SNAP_GUIDE_G, SNAP_GUIDE_B = 0.30, 0.55, 0.90
...
function QUI_LayoutMode_UI:ApplySnap(handle)
    if not self.snapEnabled then return end
    ...
    local activeThreshold = IsShiftKeyDown() and SNAP_THRESHOLD_ANCHOR or SNAP_THRESHOLD

    local bestSnapX, bestDistX = nil, activeThreshold + 1
    local bestSnapY, bestDistY = nil, activeThreshold + 1
    ...
    -- Check against other handles
    for key, otherHandle in pairs(um._handles) do
        if key ~= dragKey and otherHandle:IsShown() then
    ...
    if bestDistX <= activeThreshold and bestSnapX then
    ...
    if bestDistY <= activeThreshold and bestSnapY then
    ...
    -- Anchor-on-snap: compute anchor points when Shift is held and snapped to an element
    ...
    if shiftHeld then
        ...
        if snappedX and bestSnapXKey then
            anchorTargetKey = bestSnapXKey
        end
        if snappedY and bestSnapYKey then
            -- If we already have an X target, prefer it unless Y target is the same
            if not anchorTargetKey then
    ...
    if shiftHeld and anchorTargetKey then
        ...
        if snappedX and bestSnapXKey == anchorTargetKey and bestSnapXEdge then
        ...
        if snappedY and bestSnapYKey == anchorTargetKey and bestSnapYEdge then
        ...
        -- Show anchor label on handle
        if handle._anchorLabel then
            local info = ns.FRAME_ANCHOR_INFO and ns.FRAME_ANCHOR_INFO[anchorTargetKey]
            local displayName = info and info.displayName or anchorTargetKey
            handle._anchorLabel:SetText("|cff34d399>>|r " .. displayName)
...
CreateNudgeHandler = function(ui)
    ...
    nudge:SetScript("OnKeyDown", function(self, key)
        local um = ns.QUI_LayoutMode
        if not um or not um._selectedKey then
...
function QUI_LayoutMode_UI:OnSelectionChanged(key)
    -- Enable keyboard for arrow-key nudging only when a mover is selected
    if self._nudgeFrame then
        self._nudgeFrame:EnableKeyboard(key ~= nil)
    end
end
...
CreateToolbar = function(ui)
    ...
    title:SetText("|cff34D399Edit Mode|r")
