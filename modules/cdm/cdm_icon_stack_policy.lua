...
function CDMIconStackPolicy.Create(callbacks)
    ...
    local function TryActionButtonSpellCount(spellID, seenIDs)
        ...
        local spellCount = callbacks.querySpellCount and callbacks.querySpellCount(spellID) or nil
    ...
    function controller:ResolveMirrorStackText(icon)
        ...
        if SafeBoolean(m.stackTextShown) ~= false and controller:ValueIsPresent(stackText) then
            return stackText, m.stackTextSource or "Applications", true
        end

        local countText = m.cooldownChargesCount
        if controller:ValueIsPresent(countText)
            and (SafeBoolean(m.cooldownChargesShown) == true or SafeBoolean(m.chargeCountFrameShown) == true) then
    ...
    function controller:ResolveIconStackText(icon)
        ...
        local mirrorText, mirrorSource, mirrorBacked = controller:ResolveMirrorStackText(icon)
        ...
        local text = callbacks.queryDisplayCount and callbacks.queryDisplayCount(sid) or nil
    ...
    function controller:ShowIconStackText(icon, value, containerDB, reason)
        ...
        if sink and sink.Show then
            setOk, setErr, showOk, showErr = sink.Show(icon, value)
        else
    ...
    function controller:ApplyAuraCountText(icon, count, showZero, preserveWhenMissing)
        ...
        if controller:ValueIsPresent(count.sinkText) or showZero then
            if showZero or controller:TextHasDisplay(stackValue) then
                local sink = Sink()
                if sink and sink.Show then
                    sink.Show(icon, stackValue, count.source or "Applications")
                else
        ...
        if controller:TextHasDisplay(displayText) then
            local sink = Sink()
            if sink and sink.Show then
                sink.Show(icon, displayText, count.source or "Applications")
            else
    ...
    function controller:ApplyMirrorStackText(icon, mirrorState, showZero)
        ...
        if controller:ValueIsMissing(stackText) then
            return false
        end
