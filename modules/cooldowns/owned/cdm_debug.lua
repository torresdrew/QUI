...
--
-- Loaded last among the engine files in owned.xml, so it can:
...
local function DumpDebugIcon(icon)
    ...
    if icon._blizzFallbackPushed and icon._blizzCooldownFrame then
        ...
        local okTimes, startMS, durationMS = pcall(cd.GetCooldownTimes, cd)
        local okDraw, drawSwipe = pcall(cd.GetDrawSwipe, cd)
                ...
                if type(cData.ownedSpells) == "table" then
                    for i = 1, math.min(5, #cData.ownedSpells) do
                        ...
                        if entry and entry.id then
                            local sname = C_Spell and C_Spell.GetSpellName and C_Spell.GetSpellName(entry.id)
...
-- (cdm_debug.lua loads last among the engine files per owned.xml).
---------------------------------------------------------------------------
local CDMDebug = {}
