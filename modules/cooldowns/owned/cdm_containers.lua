...
function CDMContainers_API:GetContainers()
    ...
    for _, key in ipairs(BUILTIN_KEYS) do
        local builtInSettings = db[key] or ct[key]
        if builtInSettings then
            result[#result + 1] = { key = key, settings = builtInSettings }
        end
    end
...
function CDMContainers_API:RegisterDynamicLayoutElement(containerKey, settings)
    ...
    um:RegisterElement({
        ...
        setEnabled = function(val)
            ...
            ns.Registry:Refresh("cdmVisibility", "cdm")
        end,
        getFrame = function()
...
function CDMContainers_API:GetAllContainerKeys()
    local db = GetDB()
    if not db then return BUILTIN_KEYS end
    local ct = db.containers or {}

    local result = {}
    for _, key in ipairs(BUILTIN_KEYS) do
        if db[key] or ct[key] then
            result[#result + 1] = key
        end
    end
    local customKeys = {}
...
local function LayoutContainer(trackerKey)
    ...
    -- A full rebuild fires on PLAYER_REGEN_ENABLED via Registry:Refresh("ncdm").
    ...
    for i = 1, #allIcons do
        ...
        if not skipIcon and #iconsToLayout < totalCapacity then
    ...
    -- Hide icons that were skipped or exceeded capacity.
    local iconsToLayoutSet = {}
    for _, icon in ipairs(iconsToLayout) do
        iconsToLayoutSet[icon] = true
    end
    for _, icon in ipairs(allIcons) do
        if not iconsToLayoutSet[icon] then
            icon:Hide()
            icon:ClearAllPoints()
        end
    end
...
local function LayoutAuraContainer(trackerKey)
    local container = containers[trackerKey]
    if not container then return end
    if InCombatLockdown() then return end
    if _editModeActive and trackerKey ~= _forceLayoutKey then return end
    ...
    local allIcons = ns.CDMIcons and ns.CDMIcons:BuildIcons(trackerKey, container) or {}
    local iconSize = settings.iconSize or 32
    local aspectRatio = settings.aspectRatioCrop
    if aspectRatio == nil and settings.shape then
        if settings.shape == "rectangle" or settings.shape == "flat" then
            aspectRatio = 1.33
        else
            aspectRatio = 1.0
        end
    ...
    local growth = settings.growthDirection or "CENTERED_HORIZONTAL"

    local total = #allIcons
    local span = math.max(total - 1, 0) * (iconSize + padding)
    local startX = 0
    if growth == "CENTERED_HORIZONTAL" then
        startX = -span / 2
    end
    ...
    for i, icon in ipairs(allIcons) do
        ...
        local x
        if growth == "LEFT" then
            x = -((i - 1) * (iconSize + padding))
        elseif growth == "RIGHT" then
            x = (i - 1) * (iconSize + padding)
        else
            x = startX + ((i - 1) * (iconSize + padding))
        end
    ...
    local width = (total > 0) and (total * iconSize + math.max(total - 1, 0) * padding) or iconSize
    container:SetSize(width, iconHeight)
    applying[trackerKey] = false
end

local function LayoutAuraBarContainer(trackerKey)
    local container = containers[trackerKey]
    if not container then return end
    if InCombatLockdown() then return end
    if _editModeActive and trackerKey ~= _forceLayoutKey then return end
    ...
    if ns.CDMBars and ns.CDMBars.Refresh then
        ns.CDMBars:Refresh(container, settings, settings.barWidth, trackerKey)
    end
end

local function LayoutContainerByType(containerKey)
    local settings = GetTrackerSettings(containerKey)
    local containerType = settings and settings.containerType
    if containerType == "aura" and containerKey ~= "buff" then
        LayoutAuraContainer(containerKey)
    elseif containerType == "auraBar" then
        LayoutAuraBarContainer(containerKey)
    else
...
local function RefreshAll(forceSync)
    ...
    -- The engine's combatFrame calls Registry:Refresh("ncdm") on
    ...
    if forceSync then
        ...
        for _, key in ipairs(customKeys) do
            LayoutContainerByType(key)
        end
        UpdateAllLockedBars()
        if _G.QUI_UpdateCDMAnchoredUnitFrames then
            _G.QUI_UpdateCDMAnchoredUnitFrames()
        end
        ns.Registry:Refresh("cdmVisibility", "cdmMouseover")
        ns.Registry:Refresh("cooldownSwipe")
        ns.Registry:Refresh("cooldownGlows")
...
_G.QUI_ForceLayoutContainer = function(containerKey)
    ...
    LayoutContainerByType(containerKey)
    _forceLayoutKey = nil
...
_G.QUI_OnEditModeEnterCDM = function()
    ...
    for key, frame in pairs(containers) do
        if not BUILTIN_NAMES[key] and type(frame) == "table" and frame.GetObjectType then
...
NCDM.LayoutViewer = function(name, key)
    LayoutContainerByType(key or name)
end
...
function ownedEngine:Initialize()
    ...
    local RefreshCustomGlows
    local RefreshCooldownEffects = function() end
    local RefreshCooldownSwipe
    if ns._OwnedGlows then
        QUI.CustomGlows = ns._OwnedGlows
        RefreshCustomGlows = ns._OwnedGlows.RefreshAllGlows
    end
    if ns._OwnedSwipe then
        QUI.CooldownSwipe = ns._OwnedSwipe
        RefreshCooldownSwipe = ns._OwnedSwipe.Apply
    end

    if ns.Registry then
        ns.Registry:Register("cooldownEffects", {
            refresh = RefreshCooldownEffects,
            priority = 10,
            group = "cooldowns",
            importCategories = { "cdm" },
        })
        ns.Registry:Register("cooldownSwipe", {
            refresh = RefreshCooldownSwipe,
            priority = 10,
            group = "cooldowns",
            importCategories = { "cdm" },
        })
        ns.Registry:Register("cooldownGlows", {
            refresh = RefreshCustomGlows,
            priority = 10,
            group = "cooldowns",
            importCategories = { "cdm" },
        })
    ...
    eventFrame:SetScript("OnEvent", function(self, event, arg1, arg2)
        ...
        elseif event == "CINEMATIC_STOP" or event == "STOP_MOVIE" then
            ...
            C_Timer.After(0.3, function()
                ...
                RefreshAll()
                ns.Registry:Refresh("cdmVisibility", "cdm")
                ns.Registry:Refresh("cdmVisibility", "unitframes")
            end)
        end
    end)
end
...
do
    local function RegisterSettingsProviders()
        ...
        local function RefreshKeybinds()
            ns.Registry:Refresh("keybinds")
        end
        local function RefreshRotationHelper()
            ns.Registry:Refresh("keybinds", "rotationHelper")
        end
                ...
                if profile then
                    ...
                    CreateCollapsible(content, "Effects", effectsHeight, function(body)
                        ...
                        sy = U.PlaceRow(GUI:CreateFormCheckbox(body, hideLabel2, hideKey, effectsDB, function()
                            ns.Registry:Refresh("cooldownEffects")
                        end), body, sy)
                        ...
                        local function RefreshHighlighter()
                            ns.Registry:Refresh("cooldownHighlighter")
                        end
                ...
                if hudMinWidth then
                    CreateCollapsible(content, "HUD Min Width", 2 * FORM_ROW + 20, function(body)
                        ...
                        local function RefreshHUDWidth()
                            if ns.Registry:Has("ncdm") then ns.Registry:Refresh("ncdm")
