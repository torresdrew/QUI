...
-- REFRESH
---------------------------------------------------------------------------
local function RefreshCooldownHighlighter()
    local settings = GetSettings()
    if not settings or not settings.enabled then
        -- Remove all active highlights
        local iconsToClear = {}
        for icon, timer in pairs(activeHighlights) do
            iconsToClear[#iconsToClear + 1] = { icon = icon, timer = timer }
        end
        for _, item in ipairs(iconsToClear) do
...
if ns.Registry then
    ns.Registry:Register("cooldownHighlighter", {
        refresh = RefreshCooldownHighlighter,
        priority = 10,
        group = "cooldowns",
