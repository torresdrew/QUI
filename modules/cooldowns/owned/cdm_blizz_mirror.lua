...
---------------------------------------------------------------------------
local function StampAuraInstanceForCooldown(unit, ad)
    if not ad then return end

    -- ad.spellId may be a secret value in combat (UNIT_AURA payloads come
    -- from restricted execution). type(secret) is still "number", so a
    -- pure type guard is not enough — strip via IsSecretValue and skip;
    -- the next OOC bootstrap or non-restricted UNIT_AURA event re-stamps
    -- with a clean value. Indexing a table with a secret key errors.
    local okSid, sid = pcall(function() return ad.spellId end)
    local rawSid = okSid and sid or nil
    if CDMBlizzMirror.TaintLog then
        CDMBlizzMirror.TaintLog("Stamp.in",
            "unit", unit,
            "okSid", okSid,
            "rawSid", rawSid,
            "rawInstID", ad and ad.auraInstanceID,
            "rawName", ad and ad.name)
    end
    if not okSid then return end
    if issecretvalue and issecretvalue(sid) then
        if CDMBlizzMirror.TaintLog then
            CDMBlizzMirror.TaintLog("Stamp.skip-secret-sid", "unit", unit)
        end
        return
    end
    if type(sid) ~= "number" then return end

    -- Look up cdID across the aura viewer categories. Direct maps win so
    -- a buff icon binds to the aura cdID, not a parent ability cdID.
    --
    -- We do NOT gate on info.selfAura here. The earlier convention was:
    --   selfAura == true  → aura on player/pet, reject "target" stamps
    --   selfAura == false → aura on target,    reject "player" stamps
    -- That's wrong for entries like Anti-Magic Zone (selfAura=false but
    -- the buff variant in linkedSpellIDs lands on the player when standing
    -- in the zone). Blizzard's own mixin ignores selfAura and stamps
    -- whichever unit actually holds the aura.
    --
    -- Cross-pollution is bounded by CaptureAurasFromUnit's filter:
    --   player/pet → HELPFUL  (buffs only)
    --   target     → HARMFUL  (debuffs only)
    -- A given spellID is either a buff or a debuff (auras are typed at
    -- definition), so a buff sid X only flows from player/pet stamps and
    -- a debuff sid Y only flows from target stamps. The cdID it resolves
    -- to receives the appropriate-side instID; VerifyStateFreshness then
    -- queries C_UnitAuras.GetAuraDuration with the stamped s.auraUnit.
    local function probeCat(cat)
        return _directCDIDByCatSpell[cat][sid] or _cdIDByCatSpell[cat][sid]
    end
    local cdID = probeCat("buff") or probeCat("trackedBar")

    if CDMBlizzMirror.TaintLog then
        CDMBlizzMirror.TaintLog("Stamp.probe",
            "unit", unit, "sid", sid, "cdID", cdID)
    end
    ...
    -- auraInstanceID is secret-by-default in combat. Store as a Lua table
    -- value only — never used as a key, never == against another instID
    -- (== against a secret errors). The truthy check `not instID` is
    -- secret-safe because Lua's truthy test is a C-level type-tag check
    -- that doesn't compare values.
    local okI, instID = pcall(function() return ad.auraInstanceID end)
    if not okI or not instID then return end
...
local function CaptureAurasFromUnit(unit)
    ...
    -- Filter convention: target → HARMFUL (debuffs in trackedBar);
    -- player/pet → HELPFUL (BuffIcon + selfAura trackedBar).
    -- usePackedAura=true (5th arg): without it, AuraUtil.UnpackAuraData
    -- runs unpack(auraData.points or {}); points can be a secret value
    -- (which `or {}` does not catch — only nil), and unpack(secret) errors.
    local filter = (unit == "target") and "HARMFUL" or "HELPFUL"
    AuraUtil.ForEachAura(unit, filter, nil, function(ad)
        StampAuraInstanceForCooldown(unit, ad)
        return false
    end, true)
end

local function VerifyStateFreshness(s)
    ...
    for cdID, info in pairs(_cooldownInfoByID) do
        if entryMatches(cdID, info) then
            ...
            if C_CooldownViewer and C_CooldownViewer.GetCooldownViewerCooldownInfo then
                local ok, li = pcall(C_CooldownViewer.GetCooldownViewerCooldownInfo, cdID)
                if ok then liveInfo = li end
            end
...
local function DecodePotentialSecretBoolean(value)
    if issecretvalue and issecretvalue(value) then
        if C_CurveUtil and C_CurveUtil.EvaluateColorValueFromBoolean then
            local ok, scalar = pcall(C_CurveUtil.EvaluateColorValueFromBoolean, value, 1, 0)
            if ok and not (issecretvalue and issecretvalue(scalar)) and type(scalar) == "number" then
                return scalar >= 0.5
            end
        end
        return nil
...
local function CleanBool(v)
    if issecretvalue and issecretvalue(v) then
        if C_CurveUtil and C_CurveUtil.EvaluateColorValueFromBoolean then
            local ok, scalar = pcall(C_CurveUtil.EvaluateColorValueFromBoolean, v, 1, 0)
            if ok and not (issecretvalue and issecretvalue(scalar))
...
local function BindChildHooks(child, cooldownID, viewerCategoryNum)
    ...
    if cooldownFrame and cooldownFrame.SetCooldownFromDurationObject then
        pcall(hooksecurefunc, cooldownFrame, "SetCooldownFromDurationObject", function(self, durObj)
    ...
    if cooldownFrame and cooldownFrame.SetCooldown then
        pcall(hooksecurefunc, cooldownFrame, "SetCooldown", function(self)
            _activateMirror(self, "SetCooldown")
        end)
    end
    if cooldownFrame and cooldownFrame.SetCooldownFromExpirationTime then
        pcall(hooksecurefunc, cooldownFrame, "SetCooldownFromExpirationTime", function(self)
            _activateMirror(self, "SetCooldownFromExpirationTime")
        end)
    end
    if cooldownFrame and cooldownFrame.SetCooldownDuration then
        pcall(hooksecurefunc, cooldownFrame, "SetCooldownDuration", function(self)
            _activateMirror(self, "SetCooldownDuration")
        end)
    end
    if cooldownFrame and cooldownFrame.SetCooldownUNIX then
        pcall(hooksecurefunc, cooldownFrame, "SetCooldownUNIX", function(self)
            _activateMirror(self, "SetCooldownUNIX")
        end)
    end

    if cooldownFrame and cooldownFrame.Clear then
        pcall(hooksecurefunc, cooldownFrame, "Clear", function(self)
            local cdID = _ownerCooldownID(self)
            if not cdID then return end
            local s = _mirrorState[cdID]
            if not s then return end

            -- Aura-category cdIDs: Blizzard's mixin can call Clear when
            -- its internal active-aura check disagrees with reality
            -- (e.g. AMZ has selfAura=false, so the mixin looks on target
            -- and clears when it finds nothing — even while the linked
            -- buff variant 145629 is live on the player). Don't clobber
            -- our state if GetAuraDuration confirms the stamped aura is
            -- still on the unit; just refresh durObj from the live API.
            ...
            if (cat == "buff" or cat == "trackedBar")
                ...
                if ok and durObj then
                    ...
                    s.lastTouch = GetTime()
                    if CDMBlizzMirror.TaintLog then
                        CDMBlizzMirror.TaintLog("hook.Clear.skip-aura-live",
                            "cdID", cdID)
                    end
                    return
                end
    ...
    pcall(hooksecurefunc, child, "Show", function(self)
    ...
    pcall(hooksecurefunc, child, "Hide", function(self)
        local cdID = self.cooldownID
        if not cdID then return end
...
local function Walk()
    ...
    for catNum = 0, 3 do
        ...
        if viewer and viewer.GetChildren then
            local ok, cooldownIDs = pcall(C_CooldownViewer.GetCooldownViewerCategorySet, catNum, false)
...
local function HookViewerAlpha(viewer, viewerName)
    ...
    pcall(hooksecurefunc, viewer, "SetAlpha", function(self, alpha)
...
local function HookSelectionAlpha(viewer, viewerName)
    ...
    pcall(hooksecurefunc, sel, "Show", function(self)
    ...
    pcall(hooksecurefunc, sel, "SetAlpha", function(self, alpha)
        if _viewersSuppressed and alpha and alpha > 0 then
            C_Timer.After(0, function()
                if _viewersSuppressed and IsCDMMasterEnabled() and self:GetAlpha() > 0 then
                    self:SetAlpha(0)
...
local function DisableViewerChildrenMouse(viewer)
    ...
    for i = 1, n do
        ...
        if child then
            ...
            if child.SetMouseClickEnabled then pcall(child.SetMouseClickEnabled, child, false) end
            if child.SetMouseMotionEnabled then pcall(child.SetMouseMotionEnabled, child, false) end
        end
    end
end
        ...
        if viewer then
            ...
            if viewer.Selection and viewer.Selection.GetAlpha
               and viewer.Selection:GetAlpha() > 0 then
                pcall(viewer.Selection.SetAlpha, viewer.Selection, 0)
            end
...
local function SuppressViewers()
    ...
    for catNum = 0, 3 do
        ...
        if viewer then
            pcall(viewer.SetAlpha, viewer, 0)
            if viewer.EnableMouse then pcall(viewer.EnableMouse, viewer, false) end
            if viewer.SetMouseClickEnabled then pcall(viewer.SetMouseClickEnabled, viewer, false) end
            if viewer.SetMouseMotionEnabled then pcall(viewer.SetMouseMotionEnabled, viewer, false) end
            DisableViewerChildrenMouse(viewer)
...
UnsuppressViewers = function()
    ...
    for catNum = 0, 3 do
        ...
        if viewer then
            ...
            if viewer.EnableMouse then pcall(viewer.EnableMouse, viewer, true) end
            if viewer.SetMouseClickEnabled then pcall(viewer.SetMouseClickEnabled, viewer, true) end
            if viewer.SetMouseMotionEnabled then pcall(viewer.SetMouseMotionEnabled, viewer, true) end
            ...
            if viewer.Selection then
                pcall(viewer.Selection.SetAlpha, viewer.Selection, 1)
            end
            ...
            if ok and n then
                for i = 1, n do
                    local child = select(i, viewer:GetChildren())
                    if child then
                        if child.EnableMouse then pcall(child.EnableMouse, child, true) end
                        if child.SetMouseClickEnabled then pcall(child.SetMouseClickEnabled, child, true) end
                        if child.SetMouseMotionEnabled then pcall(child.SetMouseMotionEnabled, child, true) end
                    end
                end
            end
        end
...
---------------------------------------------------------------------------
local _hostByCooldownID    = {}    -- [cooldownID] = QUI host frame
...
-- `cd._quiIntendedSwipeColor` (set by swipe.lua when QUI applies styling)
...
local function HookSwipeStyleDefense(cd)
    ...
    if cd.SetSwipeColor then
        pcall(hooksecurefunc, cd, "SetSwipeColor", function(self, r, g, b, a)
            ...
            pcall(self.SetSwipeColor, self, want[1], want[2], want[3], wa)
        end)
    end
    if cd.SetDrawSwipe then
        pcall(hooksecurefunc, cd, "SetDrawSwipe", function(self, draw)
            local want = self._quiIntendedDrawSwipe
            if want == nil then return end
            if draw == want then return end
            pcall(self.SetDrawSwipe, self, want)
        end)
    end
    if cd.SetDrawEdge then
        pcall(hooksecurefunc, cd, "SetDrawEdge", function(self, draw)
            local want = self._quiIntendedDrawEdge
            if want == nil then return end
            if draw == want then return end
            pcall(self.SetDrawEdge, self, want)
        end)
    end
    if cd.SetSwipeTexture then
        pcall(hooksecurefunc, cd, "SetSwipeTexture", function(self, texture)
            local want = self._quiIntendedSwipeTexture
            if not want then return end
            if texture == want then return end
            pcall(self.SetSwipeTexture, self, want)
        end)
    end
end
...
local function HookHostFrameLevel(host)
    ...
    pcall(hooksecurefunc, host, "SetFrameLevel", function(self)
        ...
        if child:GetFrameLevel() ~= target then
            pcall(child.SetFrameLevel, child, target)
        end
    end)
end
...
local function ReparentSubframe(sub, host, levelOffset)
    ...
    if sub.SetParent and sub:GetParent() ~= host then
        pcall(sub.SetParent, sub, host)
    end
    if host.GetFrameStrata and sub.SetFrameStrata then
        pcall(sub.SetFrameStrata, sub, host:GetFrameStrata() or "MEDIUM")
    end
    if sub.ClearAllPoints then pcall(sub.ClearAllPoints, sub) end
    if sub.SetAllPoints  then pcall(sub.SetAllPoints, sub, host) end
    if levelOffset and host.GetFrameLevel and sub.SetFrameLevel then
        pcall(sub.SetFrameLevel, sub, (host:GetFrameLevel() or 1) + levelOffset)
    end
    if sub.EnableMouse           then pcall(sub.EnableMouse,           sub, false) end
    if sub.SetMouseClickEnabled  then pcall(sub.SetMouseClickEnabled,  sub, false) end
    if sub.SetMouseMotionEnabled then pcall(sub.SetMouseMotionEnabled, sub, false) end
end

local function FindFirstFontString(owner)
    if not owner then return nil end
    if owner.GetObjectType then
        local okType, kind = pcall(owner.GetObjectType, owner)
...
local function HideDecorativeRegion(region)
    ...
    if region.GetAtlas then
        local okAtlas, value = pcall(region.GetAtlas, region)
...
local function StripIconChrome(owner, visited, depth)
    ...
    if owner.GetNumMaskTextures and owner.GetMaskTexture and owner.RemoveMaskTexture then
        local ok, count = pcall(owner.GetNumMaskTextures, owner)
        if ok and count and count > 0 then
            for i = count, 1, -1 do
                local okGet, mask = pcall(owner.GetMaskTexture, owner, i)
                if okGet and mask then
                    pcall(owner.RemoveMaskTexture, owner, mask)
...
local function ApplyChildToHost(child, host)
    ...
    if child.DebuffBorder and child.DebuffBorder.Hide then
        pcall(child.DebuffBorder.Hide, child.DebuffBorder)
    end
...
local function HookRefreshData()
    ...
    pcall(hooksecurefunc, CooldownViewerMixin, "RefreshData", ReHostActivePool)
...
local function SafeCall(owner, method, ...)
    ...
    local ok, a, b, c, d = pcall(fn, owner, ...)
...
function HandlePlayerTotemUpdate()
    ...
    for slot = 1, maxSlots + 1 do
        local tok, hasTotem, totemName, _, _, totemIcon = pcall(GetTotemInfo, slot)
            ...
            if cdID then
                ...
                if s then
                    ...
                    if type(GetTotemDuration) == "function" then
                        ...
                        if dok and durObj and type(durObj) ~= "number" then
                            s.durObj = durObj
                        end
                    end
                end
...
_eventFrame:SetScript("OnEvent", function(self, event, arg1, arg2)
    if event == "UNIT_AURA" then
        ...
        -- to. isFullUpdate carries no addedAuras list (it's a "rescan
        -- everything" signal), so walk the live unit aura state via
        -- AuraUtil.ForEachAura. removed/updated instIDs are intentionally
        -- not handled — VerifyStateFreshness lazy-clears stale states on
    ...
    if event == "PLAYER_TARGET_CHANGED" then
        ...
        for cdID, s in pairs(_mirrorState) do
            if s.auraUnit == "target" then
                ...
                s.lastTouch      = GetTime()
                if CDMBlizzMirror.TaintLog then
                    CDMBlizzMirror.TaintLog("target.invalidate", "cdID", cdID)
                end
            end
        end
    ...
    -- the entire spellID -> cooldownID -> info graph may have changed;
    -- a full Walk re-snapshots every entry. PLAYER_LOGIN /
    -- PLAYER_ENTERING_WORLD / PLAYER_SPECIALIZATION_CHANGED / TRAIT_TREE_CHANGED
    -- can also reshape the catalog (spec change moves spells in/out of
    -- viewers; talent change rewires linkedSpellIDs).
    if event == "COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED" then
        ...
        RefreshSpellOverridePair(arg1, arg2)
        return
    end
