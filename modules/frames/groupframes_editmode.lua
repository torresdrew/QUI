    ...
    if prev and prev.defensiveIndicator then
        ...
        if defSettings and defSettings.enabled ~= false then
            ...
            if growDir == "CENTER" then
                ...
                defCenterOff = (-totalSpan / 2) + (iconSize / 2)
            end
...
local function DestroyTestFrames(onlyType)
    local function RemoveFromGlobalTestFrames(obj)
        if not obj then return end
        for idx = #testFrames, 1, -1 do
            if testFrames[idx] == obj then
                table.remove(testFrames, idx)
            end
        end
    end
    ...
    if onlyType then
        ...
        if frames then
            for _, frame in ipairs(frames) do
                frame:Hide()
                frame:SetParent(nil)
                frame._reuseContainer = nil
                RemoveFromGlobalTestFrames(frame)
            end
            wipe(frames)
        end
        ...
        if keepContainer then
            RemoveFromGlobalTestFrames(keepContainer)
            keepContainer._reuseContainer = nil
        end
        -- Update legacy refs
...
function QUI_GFEM:EnableTestMode(previewType)
    ...
    if groupMover then
        local moverChildren = { groupMover:GetChildren() }
        for _, child in ipairs(moverChildren) do
            children[#children + 1] = child
        end
    end
    if raidMover then
        local moverChildren = { raidMover:GetChildren() }
        for _, child in ipairs(moverChildren) do
            children[#children + 1] = child
        end
    end
