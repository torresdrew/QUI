    ...
    if refreshKey then
        C_Timer.After(0, function()
            if ns.Registry and ns.Registry.Refresh then
                ns.Registry:Refresh("castbar", "single", refreshKey)
            end
        end)
    end
end
...
function QUI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, unitFrame)
    ...
    if newCastbar then
        ...
        -- The debounced reapply from Registry:HookRefresh handles eventual
        ...
        for _, info in ipairs(CASTBAR_ELEMENTS) do
            um:RegisterElement({
                ...
                setEnabled = function(val)
                    ...
                    ns.Registry:Refresh("castbar", "single", info.unit)
                end,
                getFrame = function()
