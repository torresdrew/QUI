...
local function ResolveColorMode(cfg)
    local mode = cfg and cfg.colorMode
    if mode == "power" or mode == "class" or mode == "custom" then
        return mode
    end
    if cfg and cfg.useClassColor then
        return "class"
    end
    if cfg and cfg.useCustomColor and cfg.customColor then
        return "custom"
    end
    return "power"
end

local function ApplyBarColor(statusBar, cfg, resource)
    local mode = ResolveColorMode(cfg)
    if mode == "class" then
        local _, class = UnitClass("player")
        ...
        mode = "power"
    elseif mode == "custom" and cfg and cfg.customColor then
        local c = cfg.customColor
        statusBar:SetStatusBarColor(c[1], c[2], c[3], c[4] or 1)
        return
    end
...
function QUICore:UpdatePowerBar()
    ...
    ApplyBarColor(bar.StatusBar, cfg, resource)
...
function QUICore:UpdateFragmentedPowerDisplay(bar, resource, isVertical)
    ...
    local colorMode = ResolveColorMode(cfg)
    local color = GetResourceColor(resource)
    ...
    elseif colorMode == "custom" and cfg.customColor then
        local c = cfg.customColor
...
function QUICore:RefreshAll()
    ...
    for _, name in ipairs(self.viewers or {}) do
...
do
    ...
    local function TryRegisterLayoutModeElements()
        if layoutElementsRegistered then return true end
        layoutElementsRegistered = RegisterLayoutModeElements() and true or false
        return layoutElementsRegistered
    end
    if not TryRegisterLayoutModeElements() and C_Timer and C_Timer.NewTicker then
        local ticker
        ticker = C_Timer.NewTicker(0.5, function()
            if TryRegisterLayoutModeElements() and ticker then
                ticker:Cancel()
            end
        end)
    end
...
do
    ...
    local settingsProvidersRegistered = false
    local function TryRegisterSettingsProviders()
        if settingsProvidersRegistered then return true end
        settingsProvidersRegistered = RegisterSettingsProviders() and true or false
        return settingsProvidersRegistered
    end
    if not TryRegisterSettingsProviders() and C_Timer and C_Timer.NewTicker then
        local ticker
        ticker = C_Timer.NewTicker(0.5, function()
            if TryRegisterSettingsProviders() and ticker then
                ticker:Cancel()
            end
        end)
    end
