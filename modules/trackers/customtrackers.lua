...
function CustomTrackers:RefreshAll()
    ...
    ns.Registry:Refresh("customTrackers", "visibility")
end
...
if ns.Registry then
    ns.Registry:Register("customTrackers", {
        refresh = {
            all = RefreshCustomTrackers,
            visibility = UpdateCustomTrackersVisibility,
            mouseover = SetupCustomTrackersMouseoverDetector,
            keybinds = RefreshCustomTrackerKeybinds,
        },
        priority = 40,
        group = "trackers",
...
local function RegisterCustomTrackerProvider(barID, elementKey)
    ...
    settingsPanel:RegisterProvider(elementKey, { build = function(content, key, width)
        ...
        local function AddTrackerEntry(entryType, entryID, entryName)
            if CustomTrackers and CustomTrackers.AddEntry then
                CustomTrackers:AddEntry(barID, entryType, entryID)
            else
                if not barConfig.entries then barConfig.entries = {} end
                table.insert(barConfig.entries, { type = entryType, id = entryID, name = entryName })
            end
            Refresh()
        ...
        local function RemoveTrackerEntry(entry)
            if not entry then return end
            if CustomTrackers and CustomTrackers.RemoveEntry then
                CustomTrackers:RemoveEntry(barID, entry.type, entry.id)
            elseif barConfig.entries then
                for idx = #barConfig.entries, 1, -1 do
                    local candidate = barConfig.entries[idx]
                    if candidate and candidate.type == entry.type and candidate.id == entry.id then
                        table.remove(barConfig.entries, idx)
                        break
                    end
                end
            end
        ...
        U.CreateCollapsible(content, "Items & Spells", 6 * FORM_ROW + 40, function(body)
            ...
            dropZone:SetScript("OnReceiveDrag", function()
                ...
                if infoType == "item" then
                    ClearCursor()
                    AddTrackerEntry("item", id)
                elseif infoType == "spell" then
                    ClearCursor()
                    AddTrackerEntry("spell", id)
                end
            end)
            ...
            trinket1Btn:SetScript("OnClick", function()
                AddTrackerEntry("slot", 13, "Trinket 1")
            end)
            ...
            trinket2Btn:SetScript("OnClick", function()
                AddTrackerEntry("slot", 14, "Trinket 2")
            end)

            sy = sy - 28

            -- Entry list
            local configuredEntries = GetBarEntries(barConfig)
            if configuredEntries and #configuredEntries > 0 then
                for idx, entry in ipairs(configuredEntries) do
