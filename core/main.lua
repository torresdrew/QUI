...
function QUICore:OnProfileChanged(event, db, profileKey)
    ...
    ns.Registry:Refresh("specProfiles")
...
local function FlushQueuedLayoutModeCallbacks(core, um)
    if not core or not um then return end

    local enterQueue = core._editModeEnterCallbacks
    if type(enterQueue) == "table" and #enterQueue > 0 then
        for _, cb in ipairs(enterQueue) do
            if type(cb) == "function" then
                um:RegisterEnterCallback(cb)
            end
        end
        wipe(enterQueue)
    end

    local exitQueue = core._editModeExitCallbacks
    if type(exitQueue) == "table" and #exitQueue > 0 then
        for _, cb in ipairs(exitQueue) do
            if type(cb) == "function" then
                um:RegisterExitCallback(cb)
            end
        end
        wipe(exitQueue)
    end
...
local function GetLayoutModeAndFlush(core)
    local um = ns.QUI_LayoutMode
    if um then
        FlushQueuedLayoutModeCallbacks(core, um)
    end
    return um
end

if not QUICore._layoutModeQueueBridgeTicker and C_Timer and C_Timer.NewTicker then
    QUICore._layoutModeQueueBridgeTicker = C_Timer.NewTicker(0.2, function(ticker)
        local um = ns.QUI_LayoutMode
        if um then
            FlushQueuedLayoutModeCallbacks(QUICore, um)
            ticker:Cancel()
            QUICore._layoutModeQueueBridgeTicker = nil
        end
    end)
end
...
function QUICore:RegisterEditModeEnter(callback)
    ...
    local um = GetLayoutModeAndFlush(self)
    if um then
...
function QUICore:CreateMinimapButton()
    ...
    local dataObj = LDB:NewDataObject(ADDON_NAME, {
        ...
        OnTooltipShow = function(tooltip)
            ...
            tooltip:AddLine("Right-click to toggle Layout Mode", 1, 1, 1)
        end,
    })
