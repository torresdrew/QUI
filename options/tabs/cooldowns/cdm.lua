...
local function CreateCDMSetupPage(parent)
    ...
    local function RenderBarPreview(tabContent, trackerKey, y, PAD)
        ...
        if viewer and viewer.GetChildren then
            ...
            for _, child in ipairs(children) do
                ...
                if child and child ~= viewer.Selection and not cis.isCustomCDMIcon then
                    local iconTex = child.Icon or child.icon
                    if iconTex and (child.Cooldown or child.cooldown) then
                        if child:IsShown() or cis.ncdmHidden then
    ...
    local function BuildPrimaryPowerbarConfig(tabContent, ctx, y)
        ...
        snapPrimaryBtn:SetScript("OnClick", function()
            -- Force CDM refresh to ensure iconWidth state is current
            if _G.QUI_RefreshNCDM then
                _G.QUI_RefreshNCDM()
            end
        ...
        local function CalculateSnapPosition(viewer, barConfig, targetType, orientation)
            ...
            if targetType == "essential" then
                if isVertical then
                    ...
                    local totalHeight = vs.totalHeight or viewer:GetHeight() or 100
                    local topBottomBorderSize = vs.row1BorderSize or 0
                    local targetWidth = totalHeight + (2 * topBottomBorderSize) - (2 * barBorderSize)
                    local totalWidth = vs.iconWidth or viewer:GetWidth()
                    local barThickness = barConfig.height or 8
                    local rightColBorderSize = vs.bottomRowBorderSize or 0
                    ...
                    local rowWidth = vs.row1Width or vs.iconWidth or 300
                    local totalHeight = vs.totalHeight or viewer:GetHeight() or 100
                    local row1BorderSize = vs.row1BorderSize or 2
                ...
                if isVertical then
                    ...
                    local totalWidth = vs.iconWidth or viewer:GetWidth()
                    local barThickness = barConfig.height or 8
                    local row1BorderSize = vs.row1BorderSize or 0
                    ...
                    local rowWidth = vs.bottomRowWidth or vs.iconWidth or 300
                    local totalHeight = vs.totalHeight or viewer:GetHeight() or 100
                    local bottomRowBorderSize = vs.bottomRowBorderSize or 2
