...
local function CreateFrameAnchoringPage(parent)
    ...
    local function SyncSectionHeight(body, fallbackHeight)
        local realHeight = body:GetHeight()
        if not realHeight or realHeight <= 0 then
    ...
    -- CDM + HUD minimum width
    CreateCollapsible("CDM & HUD Min Width", 220, function(body)
        BuildCDMTab(body)
        SyncSectionHeight(body, 220)
    end)

    -- Resource bars
    CreateCollapsible("Resource Bars", 280, function(body)
        BuildResourceBarsTab(body)
        SyncSectionHeight(body, 280)
    end)

    -- Custom CDM bars
    CreateCollapsible("Custom CDM Bars", 420, function(body)
        BuildCustomTrackersTab(body)
        SyncSectionHeight(body, 420)
    end)
...
ns.Registry:RegisterOptions("frameAnchoring", {
    label = "Frame Positioning",
    order = 30,
    pageBuilder = CreateFrameAnchoringPage,
})
