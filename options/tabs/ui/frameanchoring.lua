...
local function BuildDisplayTab(tabContent)
    GUI:SetSearchContext({tabIndex = 3, tabName = "Frame Positioning", subTabIndex = 4, subTabName = "Display"})
    local y = -10
    -- Objective Tracker positioning moved to Edit Mode settings panel.
    local frames = {
...
local function BuildCustomTrackersTab(tabContent)
    GUI:SetSearchContext({tabIndex = 3, tabName = "Frame Positioning", subTabIndex = 6, subTabName = "Custom Trackers"})
    local y = -10
    local core = GetCore()
    local db = core and core.db and core.db.profile
