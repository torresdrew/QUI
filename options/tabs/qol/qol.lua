...
local function BuildGeneralTab(tabContent)
    ...
    local function RefreshAll()
        ...
        ns.Registry:Refresh("unitframes")
        if QUICore then
        ...
        ns.Registry:Refresh("buffBorders")
    ...
    if combatTextDB then
        CreateCollapsible("Combat Status Text Indicator", 1, function(body)
            ...
            local function RefreshCombatText()
                ns.Registry:Refresh("combatText")
            end
    ...
    if actionGeneralDB then
        ...
        CreateCollapsible("Action Tracker", 1, function(body)
            local sy = -4
            local actionPreviewActive = false
            local actionPreviewBtn

            local function RefreshActionTracker()
                ns.Registry:Refresh("actionTracker")
            end

            local function UpdateActionPreviewButtonState()
                if not actionPreviewBtn then return end
                local enabled = actionTrackerDB.enabled == true
                if actionPreviewBtn.SetEnabled then actionPreviewBtn:SetEnabled(enabled) end
            ...
            sy = P(GUI:CreateFormCheckbox(body, "Enable Action Tracker", "enabled", actionTrackerDB, function()
                if not actionTrackerDB.enabled and actionPreviewActive then
                    actionPreviewActive = false
                    if _G.QUI_ToggleActionTrackerPreview then _G.QUI_ToggleActionTrackerPreview(false) end
                    if actionPreviewBtn then actionPreviewBtn:SetText("Show Preview") end
                end
                RefreshActionTracker()
                UpdateActionPreviewButtonState()
            end), body, sy)
            sy = P(GUI:CreateFormCheckbox(body, "Only Show In Combat", "onlyInCombat", actionTrackerDB, RefreshActionTracker), body, sy)
            sy = P(GUI:CreateFormCheckbox(body, "Clear History On Combat End", "clearOnCombatEnd", actionTrackerDB, RefreshActionTracker), body, sy)

            local inactivitySlider
            local clearOnInactivityCheck
            sy = P(GUI:CreateFormCheckbox(body, "Enable Inactivity Fade-Out", "inactivityFadeEnabled", actionTrackerDB, function()
            ...
            inactivitySlider = GUI:CreateFormSlider(body, "Inactivity Timeout (sec)", 10, 60, 1, "inactivityFadeSeconds", actionTrackerDB, RefreshActionTracker)
            if inactivitySlider.SetEnabled then inactivitySlider:SetEnabled(actionTrackerDB.inactivityFadeEnabled == true) end
            sy = P(inactivitySlider, body, sy)

            clearOnInactivityCheck = GUI:CreateFormCheckbox(body, "Clear History After Inactivity", "clearOnInactivity", actionTrackerDB, RefreshActionTracker)
            if clearOnInactivityCheck.SetEnabled then clearOnInactivityCheck:SetEnabled(actionTrackerDB.inactivityFadeEnabled == true) end
            sy = P(clearOnInactivityCheck, body, sy)

            sy = P(GUI:CreateFormCheckbox(body, "Show Failed/Interrupted Casts", "showFailedCasts", actionTrackerDB, RefreshActionTracker), body, sy)
            sy = P(GUI:CreateFormSlider(body, "Max Entries", 3, 10, 1, "maxEntries", actionTrackerDB, RefreshActionTracker), body, sy)
            sy = P(GUI:CreateFormSlider(body, "Icon Size", 16, 64, 1, "iconSize", actionTrackerDB, RefreshActionTracker), body, sy)
            sy = P(GUI:CreateFormSlider(body, "Icon Spacing", 0, 24, 1, "iconSpacing", actionTrackerDB, RefreshActionTracker), body, sy)
            ...
            sy = P(GUI:CreateFormCheckbox(body, "Hide Icon Borders", "iconHideBorder", actionTrackerDB, function()
                UpdateIconBorderControls()
                RefreshActionTracker()
            end), body, sy)

            iconUseClassBorderCheck = GUI:CreateFormCheckbox(body, "Use Class Color for Icon Borders", "iconBorderUseClassColor", actionTrackerDB, function()
                UpdateIconBorderControls()
                RefreshActionTracker()
            end)
            sy = P(iconUseClassBorderCheck, body, sy)

            iconBorderColorPicker = GUI:CreateFormColorPicker(body, "Icon Border Color", "iconBorderColor", actionTrackerDB, RefreshActionTracker)
            sy = P(iconBorderColorPicker, body, sy)
            UpdateIconBorderControls()
            ...
            sy = P(GUI:CreateFormDropdown(body, "Bar Orientation", orientationOptions, "orientation", actionTrackerDB, RefreshActionTracker), body, sy)
            sy = P(GUI:CreateFormCheckbox(body, "Invert Scroll Direction", "invertScrollDirection", actionTrackerDB, RefreshActionTracker), body, sy)

            -- Container backdrop/border
            local backdropColorPicker
            sy = P(GUI:CreateFormCheckbox(body, "Show Container Background", "showBackdrop", actionTrackerDB, function()
            ...
            backdropColorPicker = GUI:CreateFormColorPicker(body, "Container Background Color", "backdropColor", actionTrackerDB, RefreshActionTracker)
            if backdropColorPicker.SetEnabled then backdropColorPicker:SetEnabled(actionTrackerDB.showBackdrop == true) end
            sy = P(backdropColorPicker, body, sy)

            local borderColorPicker
            local borderSizeSlider
            sy = P(GUI:CreateFormCheckbox(body, "Hide Container Border", "hideBorder", actionTrackerDB, function()
                ...
                if borderColorPicker and borderColorPicker.SetEnabled then borderColorPicker:SetEnabled(enabled) end
                RefreshActionTracker()
            end), body, sy)

            borderSizeSlider = GUI:CreateFormSlider(body, "Border Size", 0, 5, 0.5, "borderSize", actionTrackerDB, RefreshActionTracker)
            if borderSizeSlider.SetEnabled then borderSizeSlider:SetEnabled(actionTrackerDB.hideBorder ~= true) end
            sy = P(borderSizeSlider, body, sy)

            borderColorPicker = GUI:CreateFormColorPicker(body, "Container Border Color", "borderColor", actionTrackerDB, RefreshActionTracker)
            if borderColorPicker.SetEnabled then borderColorPicker:SetEnabled(actionTrackerDB.hideBorder ~= true) end
            sy = P(borderColorPicker, body, sy)

            sy = P(GUI:CreateFormSlider(body, "X Position Offset", -2000, 2000, 1, "xOffset", actionTrackerDB, RefreshActionTracker), body, sy)
            sy = P(GUI:CreateFormSlider(body, "Y Position Offset", -2000, 2000, 1, "yOffset", actionTrackerDB, RefreshActionTracker), body, sy)

            -- Spell blocklist
            local blocklistField = GUI:CreateFormEditBox(body, "Spell Blocklist IDs", "blocklistText", actionTrackerDB, RefreshActionTracker, {
                maxLetters = 300,
                live = true,
            ...
            local blocklistHelp = GUI:CreateLabel(body, "Comma-separated spell IDs to ignore in the tracker.", 11, C.textMuted)
            blocklistHelp:SetPoint("TOPLEFT", 0, sy + 4)
            blocklistHelp:SetPoint("RIGHT", body, "RIGHT", 0, 0)
            blocklistHelp:SetJustifyH("LEFT")
            blocklistHelp:SetWordWrap(true)
            blocklistHelp:SetHeight(16)
            sy = sy - 22

            actionPreviewBtn = GUI:CreateButton(body, "Show Preview", 140, 24)
            actionPreviewBtn:SetPoint("TOPLEFT", 0, sy)
            actionPreviewBtn:SetScript("OnClick", function(self)
                ...
                if _G.QUI_ToggleActionTrackerPreview then _G.QUI_ToggleActionTrackerPreview(actionPreviewActive) end
            ...
            -- 25 widgets + desc(25) + blocklist(FORM_ROW+22) + preview(32)
            local section = body:GetParent()
            section._contentHeight = 25 + 25 * FORM_ROW + FORM_ROW + 22 + 32 + 8
        end)
    end
    ...
    if generalDB then
        CreateCollapsible("Automation", 1, function(body)
            ...
            sy = P(GUI:CreateFormCheckbox(body, "Auto Combat Log in M+", "autoCombatLog", generalDB), body, sy)
            sy = P(GUI:CreateFormCheckbox(body, "Auto Combat Log in Raids", "autoCombatLogRaid", generalDB), body, sy)
            ...
            P(GUI:CreateFormCheckbox(body, "Auto-Enable Current Expansion Filter in Auction House", "auctionHouseExpansionFilter", generalDB), body, sy)

            local section = body:GetParent()
            section._contentHeight = 25 + 15 * FORM_ROW + 8
        end)
    end
    ...
    if generalDB then
        ...
        CreateCollapsible("Popup & Toast Blocker", 1, function(body)
            ...
            local function RefreshPopupBlocker()
                ns.Registry:Refresh("popupBlocker")
            end
    ...
    if qsDB then
        CreateCollapsible("Quick Salvage", 3 * FORM_ROW + 8, function(body)
            ...
            sy = P(GUI:CreateFormCheckbox(body, "Enable Quick Salvage", "enabled", qsDB, function()
                ns.Registry:Refresh("quickSalvage")
            end), body, sy)
    ...
    if generalDB then
        CreateCollapsible("Consumable Check", 1, function(body)
            ...
            local function RefreshConsumables()
                ns.Registry:Refresh("consumables")
            end
    ...
    CreateCollapsible("QUI Panel Settings", 1, function(body)
        ...
        if minimapBtnDB then
            sy = P(GUI:CreateFormCheckbox(body, "Hide QUI Minimap Icon", "hide", minimapBtnDB, function(dbVal)
                ...
                ns.Registry:Refresh("minimap", "buttonDrawer")
            end), body, sy)
        end
