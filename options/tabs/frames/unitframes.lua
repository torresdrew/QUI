...
local function CreateUnitFramesPage(parent)
    ...
    local function BuildGeneralTab(tabContent)
        ...
        local function relayout()
            local cy = -10
            -- Enable checkbox is first (always visible, stored as sections[0])
            if sections[0] then
                sections[0]:ClearAllPoints()
                sections[0]:SetPoint("TOPLEFT", PAD, cy)
                sections[0]:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
                cy = cy - FORM_ROW
            end
            for _, s in ipairs(sections) do
                s:ClearAllPoints()
                s:SetPoint("TOPLEFT", PAD, cy)
                s:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
                cy = cy - s:GetHeight() - 4
            end
            tabContent:SetHeight(math.abs(cy) + 20)
        end

        local function CreateCollapsible(title, contentHeight, buildFunc)
            local section = CreateFrame("Frame", nil, tabContent)
