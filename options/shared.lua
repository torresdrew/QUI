...
local function RefreshCrosshair()
    ns.Registry:Refresh("crosshair")
end

local function RefreshReticle()
    ns.Registry:Refresh("reticle")
end

local function RefreshRangeCheck()
    ns.Registry:Refresh("rangecheck")
end
...
local function CreateCollapsiblePage(parent, pad, topOffset)
    ...
    local function CreateCollapsible(title, contentHeight, buildFunc)
        ...
        local originalSetHeight = body.SetHeight
        if originalSetHeight and not body._quiSetHeightWrapped then
            body._quiSetHeightWrapped = true
            body.SetHeight = function(self, height)
                originalSetHeight(self, height)
                section._contentHeight = height or self:GetHeight() or 0
                if section._expanded then
                    section:SetHeight(COLLAPSIBLE_HEADER_HEIGHT + section._contentHeight)
                end
                relayout()
        ...
        -- Sync to built content in case buildFunc changed body height directly.
        section._contentHeight = body:GetHeight()
